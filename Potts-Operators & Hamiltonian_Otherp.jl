...
H_2(; kwargs...) = H_2(ComplexF64, Trivial; kwargs...)
H_2(elt::Type{<:Number}; kwargs...) = H_2(elt, Trivial; kwargs...)
function H_2(symmetry::Type{<:Sector}; kwargs...)
    return H_2(ComplexF64, symmetry; kwargs...)
end


function H_2(elt::Type{<:Number}, ::Type{Trivial}; q=5,k=1)
    pspace = ComplexSpace(q)
    sigma = TensorMap(zeros, elt, pspace ← pspace)
    tau = TensorMap(zeros, elt, pspace ← pspace)
    identity_e = TensorMap(zeros, elt, pspace ← pspace)
    ...
    return (tau^k⊗tau^k⊗ identity_e) +  (sigma'⊗  identity_e⊗ sigma)^k
end
function Potts_H_alt(L; J=1,h=1,Q=5,lambda=0)
    ### symmetry  
            H = periodic_boundary_conditions(quantum_potts(ZNIrrep{Q};q=Q), L)
            _,_,W = weyl_heisenberg_matrices(Q)
            P   = TensorMap(W,ℂ^Q←ℂ^Q)
            ...
            # dat0 = reshape((P'*sum((-h * potts_spin_shift(; q = Q,k=j)) for j in 1:1:Q-1)*P).data, (Q,Q))
            # dat1 = reshape(((P' ⊗ P')*sum((-J * potts_phase(; q=Q,k=j)) for j in 1:1:Q-1)*(P ⊗ P)).data, (Q,Q,Q,Q))
            dat2 = reshape(((P' ⊗ P'⊗ P') * sum(H_2(;q=Q,k=l) for l in 1:1:Q-1)*(P⊗P⊗P)).data, (Q,Q,Q,Q,Q,Q))
            dat2 = TensorMap(dat2,Vp⊗Vp⊗Vp←Vp⊗Vp⊗Vp)
            # H0 = @mpoham (sum(TensorMap(dat0,Vp←Vp){i} for i in vertices(lat)[1:(end)])) ### Potts
            # H1 = @mpoham (sum(TensorMap(dat1, Vp⊗Vp←Vp⊗Vp){i,i+1}  for i in vertices(lat)[1:(end-1)]) + TensorMap(dat1, Vp⊗Vp←Vp⊗Vp){vertices(lat)[end],vertices(lat)[1]}) ##¨Potts with BC
            H2 =  @mpoham  (sum(dat2{i,i+1,i+2} for i in vertices(lat)[1:(end - 2)]) + dat2{vertices(lat)[end],vertices(lat)[1],vertices(lat)[2]}+ dat2{vertices(lat)[end-1],vertices(lat)[end],vertices(lat)[1]}) ###Extra term
            # ham = H0+H1+H2
            ham = H + lambda *H2
    return ham
end