...
function lambda_estimation(L,lambda_range,alg)
    ...
    for (i,lambda) in enumerate(lambda_range)
        ...
        if alg == "QuasiparticleAnsatz"
            ...
            st = vcat(st1,st0)
            stt = vcat([ψ,],st)
            save_object("OBC L = $L,5statesQP$lambda.jld2", stt)
    ...
    save_object("$alg OBC Lambda_est_ge_woc$L.jld2", gε_prime_wo_C)
    save_object("$alg OBC Lambda_est_ge$L.jld2", gε_prime)
...
distx = 0.001## distance from alleged fixed point 0.079+0.060i in real
disty = 0.001# distance from alleged fixed point 0.079+0.060i in imaginary
cent_im = 0.058im
cent_r = 0.078
